...
// Use vi.hoisted for mock functions used in vi.mock
const { mockSendRequest, mockRenderRequest } = vi.hoisted(() => ({
  mockSendRequest: vi.fn(),
  mockRenderRequest: vi.fn(),
}));

// Mock HttpService as a class
vi.mock('../http/client', () => ({
...
vi.mock('../utils/template-engine', () => ({
  TemplateEngine: {
    renderRequest: mockRenderRequest,
  },
}));
...
describe('RequestExecutionService', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(models.getActiveEnvironment).mockResolvedValue({
      _id: 'e1',
      name: 'Env',
      workspaceId: 'w1',
      isActive: true,
    ...
    vi.mocked(models.getVariablesByEnvironment).mockResolvedValue([
      {
        _id: 'v1',
        key: 'VAR',
        value: 'VAL',
    ...
    vi.mocked(models.createResponse).mockResolvedValue({
      ...mockResponse,
      type: 'Response',
      requestId: 'req1',
      bodyPath: '/path/to/body',
      created: 0,
      modified: 0,
    });
    vi.mocked(models.getSettings).mockResolvedValue({
      _id: 'settings',
      type: 'Settings',
      timeout: 30000,
      ...
      theme: 'auto',
      fontSize: 14,
      maxHistoryResponses: 50,
      created: 0,
      modified: 0,
    });

    mockRenderRequest.mockReturnValue({ ...mockRequest });
  ...
  it('should execute a simple request successfully', async () => {
    ...
    expect(mockRenderRequest).toHaveBeenCalled();
  ...
  it('should run pre-request script if present', async () => {
    ...
    mockRenderRequest.mockReturnValue(req);
  ...
  it('should calculate Host header if auto-header is present', async () => {
    ...
    const sentReq = mockSendRequest.mock.calls[0][0];
  ...
  it('should run assertions if present', async () => {
    const req = {
      ...mockRequest,
      assertions: [{ id: 'a1', source: 'status', operator: 'equals', value: '200', enabled: true }],
    };
    mockRenderRequest.mockReturnValue(req);
    ...
    vi.mocked(assertionService.runAssertions).mockReturnValue({
      ...
      failed: 0,
      total: 1,
      results: [{ assertionId: 'a1', status: 'pass' }],
    });

    await RequestExecutionService.executeRequest(req as any);

    expect(assertionService.runAssertions).toHaveBeenCalled();
    expect(models.createResponse).toHaveBeenCalledWith(
  ...
  it('should handle errors gracefully', async () => {
    ...
    await expect(RequestExecutionService.executeRequest(mockRequest as any)).rejects.toThrow(
      'Network Error'
    );
  });
  ...
  it('should update variables from scripts', async () => {
    ...
    vi.mocked(ScriptService.executeScript).mockImplementation(async (_script, ctx) => {
