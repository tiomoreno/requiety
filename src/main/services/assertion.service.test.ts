...
describe('AssertionService', () => {
  ...
  describe('runAssertions', () => {
    it('should return empty results if no assertions provided', async () => {
      const result = await AssertionService.runAssertions(mockResponse, []);
      expect(result.total).toBe(0);
      expect(result.passed).toBe(0);
      expect(result.failed).toBe(0);
    });

    it('should ignore disabled assertions', async () => {
      ...
      const result = await AssertionService.runAssertions(mockResponse, assertions);
      expect(result.total).toBe(0);
    });

    describe('Status Code Assertions', () => {
      it('should pass checks for correct status code', async () => {
        const assertions: Assertion[] = [
      ...
      it('should fail checks for incorrect status code', async () => {
        const assertions: Assertion[] = [
    ...
    describe('Header Assertions', () => {
      it('should verify header existence and value', async () => {
        const assertions: Assertion[] = [
    ...
    describe('Response Time Assertions', () => {
      it('should verify response time', async () => {
        const assertions: Assertion[] = [
    ...
    describe('JSON Body Assertions', () => {
      it('should verify nested json properties', async () => {
        const assertions: Assertion[] = [
          { id: '1', source: 'jsonBody', property: '$.user.name', operator: 'equals', value: 'John Doe', enabled: true },
          { id: '2', source: 'jsonBody', property: '$.user.roles', operator: 'contains', value: 'admin', enabled: true }
        ];
        const result = await AssertionService.runAssertions(mockResponse, assertions);
        expect(result.passed).toBe(2);
      });

      it('should handle non-json bodies gracefully', async () => {
        const textResponse: Response = { ...mockResponse, body: 'Not JSON' };
        const assertions: Assertion[] = [
           { id: '1', source: 'jsonBody', property: '$.foo', operator: 'exists', enabled: true }
        ];
        const result = await AssertionService.runAssertions(textResponse, assertions);
        expect(result.failed).toBe(1);
        // It catches JSON parse error or logic error and returns fail
        expect(result.results[0].status).toBe('fail');
      });
      
      it('should support array existence checks via jsonpath', async () => {
         const assertions: Assertion[] = [
