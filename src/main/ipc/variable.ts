...
export const registerVariableHandlers = (): void => {
  ...
  ipcMain.handle(
    ...
    async (_, data: Pick<Variable, 'environmentId' | 'key' | 'value' | 'isSecret'>) => {
      try {
        const variable = await createVariable(data);
        return { success: true, data: variable };
      } catch (error) {
        LoggerService.error('Error creating variable:', error);
        return {
          success: false,
  ...
  ipcMain.handle(
    ...
    async (_, id: string, data: Partial<Pick<Variable, 'key' | 'value' | 'isSecret'>>) => {
      try {
        const variable = await updateVariable(id, data);
        return { success: true, data: variable };
      } catch (error) {
        LoggerService.error('Error updating variable:', error);
        return {
          success: false,
          error: error instanceof Error ? error.message : 'Unknown error',
        };
  ...
  ipcMain.handle(IPC_CHANNELS.VARIABLE_DELETE, async (_, id: string) => {
    try {
      ...
      LoggerService.error('Error deleting variable:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      };
  ...
  ipcMain.handle(IPC_CHANNELS.VARIABLE_GET_BY_ENVIRONMENT, async (_, environmentId: string) => {
    try {
      const variables = await getVariablesByEnvironment(environmentId);
      return { success: true, data: variables };
    } catch (error) {
      LoggerService.error('Error getting variables:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      };
