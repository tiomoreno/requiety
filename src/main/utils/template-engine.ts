...
export class TemplateEngine {
  ...
  static render(template: string, variables: Record<string, string>): string {
    ...
    try {
      ...
      LoggerService.warn('Template render error:', error);
      return template;
    }
  }
  ...
  static renderRequest(request: any, variables: Variable[]): any {
    ...
    if (rendered.body) {
      if (
        rendered.body.type === 'json' ||
        rendered.body.type === 'raw' ||
        rendered.body.type === 'text'
      ) {
        ...
        (rendered.body.type === 'form-data' || rendered.body.type === 'form-urlencoded') &&
        rendered.body.params
      ) {
        rendered.body.params = rendered.body.params.map(
          (param: { name: string; value: string; enabled: boolean }) => ({
            ...param,
    ...
    if (rendered.authentication) {
      ...
      if (rendered.authentication.type === 'basic') {
        if (rendered.authentication.username)
          rendered.authentication.username = this.render(rendered.authentication.username, context);
        if (rendered.authentication.password)
          rendered.authentication.password = this.render(rendered.authentication.password, context);
      }
    }
