...
describe('Database Models', () => {
  ...
  describe('Requests', () => {
    ...
    it('duplicateRequest should create a copy with new ID', async () => {
      ...
      // Note: sortOrder is preserved from original (1), not incremented
      expect(mockDb.insert).toHaveBeenCalledWith(
    ...
    it('getWorkspaceIdForRequest should traverse up to workspace', async () => {
      ...
      // Request lookup
      mockDb.findOne.mockImplementationOnce((q, cb) =>
        cb(null, { _id: 'r1', parentId: 'f2', type: 'Request' })
      );
      ...
      // Request r1
      requestDb.findOne.mockImplementation((q, cb) =>
        cb(null, { _id: 'r1', parentId: 'f2', type: 'Request' })
      );

      // F2
      workspaceDb.findOne.mockImplementationOnce((q, cb) => cb(null, null)); // Not workspace
      folderDb.findOne.mockImplementationOnce((q, cb) =>
        cb(null, { _id: 'f2', parentId: 'f1', type: 'Folder' })
      );

      // F1
      workspaceDb.findOne.mockImplementationOnce((q, cb) => cb(null, null)); // Not workspace
      folderDb.findOne.mockImplementationOnce((q, cb) =>
        cb(null, { _id: 'f1', parentId: 'w1', type: 'Folder' })
      );

      // W1
      workspaceDb.findOne.mockImplementationOnce((q, cb) =>
        cb(null, { _id: 'w1', type: 'Workspace' })
      );
