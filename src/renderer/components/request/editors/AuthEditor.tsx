...
export function AuthEditor({ auth, onChange }: AuthEditorProps) {
  const handleTypeChange = (type: AuthType) => {
    // Reset fields when type changes for cleanliness, or keep them if we want persistence across toggles
    // Let's reset relevant fields to defaults
    onChange({ type });
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 p-4">
      <div className="max-w-xl">
        <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Auth Type</label>
        <select
          value={auth.type}
