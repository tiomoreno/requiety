...
describe('DataTransferService', () => {
    ...
    it('exportWorkspace should return false on cancel', async () => {
        vi.mocked(window.api.importExport.exportWorkspace).mockResolvedValue({ success: false, error: 'Cancelled' });
        const result = await dataTransferService.exportWorkspace('w1');
        expect(result).toBe(false);
    });

    it('exportWorkspace should throw on error', async () => {
        vi.mocked(window.api.importExport.exportWorkspace).mockResolvedValue({ success: false, error: 'IO Error' });
    ...
    it('importWorkspace should return false on cancel', async () => {
         vi.mocked(window.api.importExport.importWorkspace).mockResolvedValue({ success: false, error: 'Cancelled' });
        const result = await dataTransferService.importWorkspace();
        expect(result).toBe(false);
    });
    
    it('importWorkspace should throw on error', async () => {
        vi.mocked(window.api.importExport.importWorkspace).mockResolvedValue({ success: false, error: 'Corrupt File' });
