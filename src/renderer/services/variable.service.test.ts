...
describe('VariableService', () => {
    ...
    it('create should return new variable', async () => {
        ...
        const result = await variableService.create({ environmentId: 'e1', key: 'K', value: 'V', isSecret: false });
    ...
    it('getByEnvironment should return variables', async () => {
        ...
        vi.mocked(window.api.variable.getByEnvironment).mockResolvedValue({ success: true, data: mockVars });
    ...
    it('getByEnvironment should return empty on success null data', async () => {
        vi.mocked(window.api.variable.getByEnvironment).mockResolvedValue({ success: true, data: null });
        const result = await variableService.getByEnvironment('e1');
