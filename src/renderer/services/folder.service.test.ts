...
describe('FolderService', () => {
  ...
  it('getByWorkspace should return folders on success', async () => {
    ...
    vi.mocked(window.api.folder.getByWorkspace).mockResolvedValue({
      success: true,
      data: mockFolders,
    });
  ...
  it('create should return new folder', async () => {
    ...
    expect(window.api.folder.create).toHaveBeenCalledWith({
      name: 'New',
      parentId: 'root',
      sortOrder: 0,
    });
  ...
  it('create should throw on failure', async () => {
    ...
    await expect(folderService.create({ name: 'N', parentId: 'p', sortOrder: 0 })).rejects.toThrow(
